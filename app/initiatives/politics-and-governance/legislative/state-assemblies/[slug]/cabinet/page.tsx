...
import { STATE_ASSEMBLY_CABINET_MEMBERS_QUERY, STATE_DETAIL_QUERY } from '@/lib/sanity/queries';
...
export default async function StateAssemblyCabinetPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const members = await client.fetch<StateAssemblyMember[]>(STATE_ASSEMBLY_CABINET_MEMBERS_QUERY, { slug });
    ...
    <main className="min-h-screen bg-slate-50">
      ...
      <section className="bg-navy py-12 px-6 text-white">
        <div className="mx-auto max-w-7xl">
          <FadeIn>
            <Link 
              href={`/initiatives/politics-and-governance/legislative/state-assemblies/${slug}`}
              className="inline-flex items-center gap-2 text-teal hover:text-white mb-6 transition-colors"
            >
              <ArrowLeft className="h-4 w-4" />
              <span>Back to {state.name} Assembly Dashboard</span>
            </Link>
            <h1 className="font-serif text-4xl md:text-5xl font-bold mb-4">
              {state.name} State House Leadership
            </h1>
            <p className="text-xl text-gray-200 max-w-2xl">
              Meet the Speaker, Principal Officers, and Honourable Members of the {state.name} State House of Assembly.
            </p>
          </FadeIn>
        </div>
      </section>
        ...
        <FadeIn>
          {members.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-2xl border border-gray-200">
              ...
              <p className="text-gray-500">The leadership list for {state.name} is currently being updated.</p>
            </div>
          ) : (
              ...
              {members.map((member) => (
                ...
                >
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                    ...
                    {member.constituency && (
                      <div className="flex items-center gap-2 text-slate font-medium text-sm mb-6">
                        <Building2 className="h-4 w-4 shrink-0 text-gold" />
                        <span>{member.constituency} Constituency</span>
                      </div>
                    )}
